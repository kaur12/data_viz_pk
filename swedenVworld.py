...
with open('data/OlympicsWinter.csv') as csvfile:
	...
	for row in reader:
		if line_count is 0:
			...
			sweden.append([int(row[0]), row[5], row[6], row[7]]) # multidemensional array
		else:
			world.append([int(row[0]), row[5], row[6], row[7]]) 
		line_count += 1

print('total medal for sweden:', len(sweden))
...
print('sweden won', len(gold_1924), 'gold medals in 1924')
print('sweden', len(gold_1972), 'gold medals in 1972')
print('sweden', len(gold_2014), 'gold medals in 2014')
...
plt.title("Medal wins by Sweden")
